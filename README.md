...
`tools/build.py` uses the 18 approved PNGs in `assets/`, creates the three abstract reference cards and synthetic collage fixtures, and bundles the standalone `demo/index.html`. It does not need the original image collection or a saved workspace. Screenshot capture uses fresh demo contexts and writes `docs/images/`.
