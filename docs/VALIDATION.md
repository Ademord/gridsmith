...
Three fresh reviewers cover experience, privacy and delivery. The experience reviewer reproduced the invitation intercepting Library expansion at 320 × 640, then verified the correction at 1440 × 1000, 390 × 844 and 320 × 640. Review also found that Help Escape cleared selection in both version 1.1 and the first candidate. Both findings are independently closed on the app hash above. Original failure evidence is retained outside the repository.

The suite now has 49 browser cases, including actual Library interaction before dismissal, persistence through reload, accessible Help re-entry, selection preservation, and blocked preference storage. Twenty-four release canaries passed. The first source commit passed all three Linux browser suites in [CI run 34748542849](https://github.com/Ademord/gridsmith/actions/runs/34748542849). Exact source-commit CI, release-download parity and live Pages journeys are separate delivery gates; the versioned release notes record their final identities and outcomes.

The separate [first Pages run](https://github.com/Ademord/gridsmith/actions/runs/34748564844) correctly stopped when Firefox's keyboard-history test read the order before asynchronous Undo completed. Its failure is retained. That test now waits within the existing time limit for the exact complete order and an available next history control, then checks persisted order too. Redo checks the complete order as well. The application and image bytes did not change for this test correction. An independent reviewer reproduced the original failure with delayed storage, ran the exact corrected callback successfully under the same delay, and verified that a deliberately broken Undo still fails within the unchanged 15-second bound.

The [corrected Pages run](https://github.com/Ademord/gridsmith/actions/runs/34748850572) passed all four verification jobs, packaging and deployment. An independent anonymous download matched the exact 5,347,579-byte app and its build-info. Hosted journeys at 1440 × 1000 and 320 × 640 passed all 17 guide steps, actual import selection/cancellation/correction, native crop pixels, Original bytes, captions/drafts, restoration into empty storage and ZIP order/dimensions. No application errors or external asset/service calls were observed. A copied offline-file preflight passed separately. Actual versioned release-download verification and the final documentation commit's CI/hosting identities belong in the [version 1.1.1 release record](https://github.com/Ademord/gridsmith/releases/tag/v1.1.1); a copied file alone does not establish that gate.
